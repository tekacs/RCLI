		...
		if (cmd_name in this.commands) {
			...
			if (!(this.fallback && this.fallback(terminal, cmd))) {
                          $.ajax({
                            ...
                            url: "update",
                            type: "POST",
                            data: ({pre_input:command_history+temporary_history,input:cmd}),
                            success: function(text){
                              switch(text.charAt(0)){
                                case "S": // Success
                                  command_history += temporary_history + cmd + "\n";
                                  temporary_history = "";
                                  terminal.print(text.substring(1));
                                break;
                                case "I": // Incomplete
                                  temporary_history += cmd + "\n";
                                break;
                                case "E": // Error
                                  terminal.print(text.substring(1));
                                break;
...
$(document).ready(function() {
	...
	Terminal.init();
        ...
        Terminal.print("Shell started - " + hours + ":" + minutes);
});
